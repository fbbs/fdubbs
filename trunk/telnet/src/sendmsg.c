...
//»ñÈ¡msg¼ÇÂ¼ÊýÄ¿,Ô­À´µÄget_num_recordÖ»ÄÜÖ§³ÖÒ»ÐÐmsg
...
//	*(timestr + 8) = '\0';
	...
	file_append("/home/bbs/msglog", temp);
        ...
        sprintf(msgbuf, "[0;1;5;44;33mÕ¾³¤ ì¶[36m %24.24s [33m¹ã²¥£º[m[1;37;44m%-39.39s[m[%05dm\n", timestr," ",  uinfo.pid); 
		sprintf(wholebuf, "%s\n", msgstr);
        strcat(msgbuf, wholebuf);        

	} else if (mode == 1) {
		//sprintf(msgbuf, "[0;1;44;36m%-12.12s[37m([36m%-5.5s[37m) ÑûÇëÄã[37m%s[31m(%s)[m[%05dm\n",
		//	currentuser.userid, timestr, msgstr, ret_str, uinfo.pid);
        sprintf(msgbuf, "[0;1;44;36m%-12.12s[37m([36m%-24.24s[37m) ÑûÇëÄã[37m%-34.34s[31m(%s)[m[%05dm\n", currentuser.userid, timestr, " ", ret_str, uinfo.pid); 
       	sprintf(wholebuf, "%s\n", msgstr); 
		strcat(msgbuf, wholebuf);
	} else if (mode == 3) {
...
/* added by roly 02.06.02 for logoff msg */
		...
		//sprintf(msgbuf,"[0;1;45;36m%-12.12s[36mÏòÄú¸æ±ð([1;36;45m%8.8s[36m)£º[m[1;36;45m%s[m[%05dm\n",
		//	currentuser.userid,timestr, msgstr, 0);
        sprintf(msgbuf, "[0;1;45;36m%-12.12s[36mÏòÄú¸æ±ð([1;36;45m%24.24s[36m)£º[m[1;36;45m%-38.38s[m[%05dm\n", currentuser.userid, timestr, " ", 0); 
		sprintf(wholebuf, "%s\n", msgstr); 
        strcat(msgbuf, wholebuf);
	}
/* add end */
...
int dowall(struct user_info *uin)
{
	...
	move(1, 0);
	clrtoeol();
	prints("[1;32mÕý¶Ô %s ¹ã²¥.... Ctrl-D Í£Ö¹¶Ô´ËÎ» User ¹ã²¥¡£[m", uin->userid);
	refresh();
...
*/
	...
	case '1':
		...
		if (apply_ulist(myfriend_wall) == -1) {
			...
			/* ¼ÇÂ¼ËÍÑ¶Ï¢¸øºÃÓÑ */
		//	sprintf(msgbuf, "[0;1;45;36mËÍÑ¶Ï¢¸øºÃÓÑ[33m([36m%-5.5s[33m):[37m%s[31m(^Z»Ø)[m[%05dm\n",
		//		timestr, buf2, uinfo.pid);
        sprintf(msgbuf, "[0;1;45;36mËÍÑ¶Ï¢¸øºÃÓÑ[33m([36m%-24.24s[33m):[37m%-39.39s[31m(^Z»Ø)[m[%05dm\n", timestr," ",  uinfo.pid); 
	...
	case '2':
		...
		if (apply_ulist(hisfriend_wall) == -1) {
			...
			//sprintf(msgbuf, "[0;1;45;36mËÍ¸øÓëÎÒÎªÓÑ[33m([36m%-5.5s[33m):[37m%s[31m(^Z»Ø)[m[%05dm\n",
			//	timestr, buf2, uinfo.pid);
...
r_msg2()
{
	...
	while (1) {
        ...
        //prints("%s", msg); //¸Ä³É¶àÐÐmsgÒÔºó£¬Ã»ÄÇÃ´¼òµ¥
	...
	file_append("/home/bbs/msglog", temp);
			...
			//modified end
				...
				sprintf(msgbuf, "[1;32mÕÒ²»µ½·¢Ñ¶Ï¢µÄ %s! Çë°´ÉÏ:[^Z ¡ü] »òÏÂ:[^A ¡ý] »òÆäËû¼üÀë¿ª.[m", usid);
				move(msg_line, 0);
                clrtoeol();
				refresh();
				prints("%s", msgbuf);
...
r_msg()
{
	...
	file_append("/home/bbs/msglog", temp);
	  ...
	  char bak_title[STRLEN];
           ...
           sprintf(mustbak_title, "[%s] Ç¿ÖÆÑ¶Ï¢±¸·Ý%dÌõ", datestring,i);
