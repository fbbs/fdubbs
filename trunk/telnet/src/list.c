...
int     showexplain = 0;char   fexp[30]; //added by kit to make W work
...
int do_userlist()
{
   ...
   for (i = 0, j = 0; j < BBS_PAGESIZE && i + page < range; i++) {
	...
	//Added IAMFAT 2002-05-27
	char userid[STRLEN];
	strcpy(userid,(override&&showexplain)?get_explain(uentp->userid):uentp->username);
	ellipsis(userid,20);
	//End IAMFAT
	 sprintf(user_info_str," [m%4d%2s%-12.22s[37m %-20.20s[m %-15.15s %c %c %c %s%-10.10s[37m %5.5s[m\n", i + 1 + page,
	//Modified by IAMFAT 2002-05-27
#ifdef SHOWMETOFRIEND	 
	    (override) ? ((hisfriend(uentp))? "[32m°Ã":"°Ã[32m"):((hisfriend(uentp))?"[33m  ":"  "), 
	    uentp->userid,userid,
...
int deal_key(char ch, int allnum, int pagenum) //ª∑πÀÀƒ∑Ω¥¶¿Ì∞¥º¸
{
   ...
   switch (ch) {
      case 'Y':
         if(HAS_PERM(PERM_CLOAK)) x_cloak();
	 break;
      case 'P':
         t_pager();
	 break;
	 ...
	 break;
      case 'k':
      case 'K':
         if (!HAS_PERM(PERM_USER) && (usernum!=user_record[allnum]->uid))
	    return 1;
	 if ( !strcmp(currentuser.userid,"guest") ) return 1;   
...
//         if (!HAS_PERM(PERM_PAGE)) return 1;
         if (!HAS_PERM(PERM_TALK)) return 1;
	 ...
	 break;
      case 'v':
      case 'V':
         if( !HAS_PERM(PERM_USER)) return 1;
...
/*         if (!HAS_PERM(PERM_POST)) return 1;*/
         if (!HAS_PERM(PERM_MAIL)) return 1;
...
/*      case 'x':
	 ...
	 break;*/ //Commented by Amigo 2002.06.07
      case 's':
      case 'S':
	 if (!strcmp("guest", currentuser.userid)) return 0;
         if (!HAS_PERM(PERM_TALK)) return 1;
   ...
   if (friendmode) modify_user_mode(FRIEND);
   else modify_user_mode(LUSERS);
   if (readplan == NA) {
      print_title();
      clrtobot();
      if (show_userlist() == -1) return -1;
      if (msgflag) show_message(buf);
      update_endline();
   }
   return 1;
}
...
/*         if (!HAS_PERM(PERM_POST)) return 1;*/
	 ...
	 msgflag = YEA;
	 if (!friendmode) return 1;
	 break;
      case 'f':
      case 'F':
         toggle1++;
	 if (toggle1 >= 3) toggle1 = 0;
	 break;
      case 'W':
      case 'w':
         if(showexplain==1)
...
{
	countusers(NULL);
	if (apply_record(PASSFILE, countusers, sizeof(struct userec),0,0,0) == -1) {
		return 0;
	}
...
int     mode;
{
	...
	mailmode = mode;
	if (apply_record(PASSFILE, mailto, sizeof(struct userec),0,0,0) == -1) {
...
Show_Users()
{
	...
	if (apply_record(PASSFILE, printuent, sizeof(struct userec),0,0,0) == -1) {
		prints("No Users Exist");
		pressreturn();
		return 0;
	}
