...
m_ordainBM (){
  ...
  if (!gettheuserid (2, "ÊäÈëÓûÈÎÃüµÄÊ¹ÓÃÕßÕÊºÅ: ", &id))
    return 0;
  if (!gettheboardname
      (3, "ÊäÈë¸ÃÊ¹ÓÃÕß½«¹ÜÀíµÄÌÖÂÛÇøÃû³Æ: ", &pos, &fh, bname, 0))
    return -1;
  if (fh.BM[0] != '\0') {
  ...
  if (find || oldbm == 3) {	//Í¬Ò»ID²»ÄÜ¼æÈÎ³¬¹ıÈı¸ö°æµÄ°æÖ÷
    move (5, 0);
    prints (" %s ÒÑ¾­ÊÇ%s°æµÄ°æÖ÷ÁË", lookupuser.userid,
	    find ? "¸Ã" : "Èı¸ö");
    pressanykey ();
    clear ();
    return -1;
  ...
  //¾«»ªÇøµÄÏÔÊ¾: ¶¯Ì¬·ÖÅä        ÏÔÊ¾10¸ö¿Õ¸ñ printf("%*c",10,' ');
  {
    ...
    sprintf (genbuf, "%s%*c(BM: %s)", fh.title + 8, blanklen, BLANK, buf[0]);
  }
  buf[0][0] = '\0';
  get_grp (fh.filename);
...
sprintf(genbuf,"\n"
 ...
 //add end
  ...
  sprintf (bmfilename, "ÈÎÃü %s Îª %s °æ%s",
	   lookupuser.userid, fh.filename, bm ? "°æÖ÷" : "°æ¸±");
...
m_retireBM ()
{
  ...
  if (!oldbm || !find) {
    move (5, 0);
    prints (" %s %s°æ°æÖ÷£¬ÈçÓĞ´íÎó£¬ÇëÍ¨Öª³ÌĞòÕ¾³¤¡£",
  ...
  if (!right) {
    move (5, 0);
    prints ("¶Ô²»Æğ£¬ %s °æ°æÖ÷Ãûµ¥ÖĞÃ»ÓĞ %s £¬ÈçÓĞ´íÎó£¬ÇëÍ¨Öª¼¼ÊõÕ¾³¤¡£",
  ...
  if (fh.BM[0] != '\0') {
    ...
    //¾«»ªÇøµÄÏÔÊ¾: ¶¯Ì¬·ÖÅä        ÏÔÊ¾10¸ö¿Õ¸ñ printf("%*c",10,' ');
    {
      ...
      sprintf (genbuf, "%s%*c(BM: %s)",
	       fh.title + 8, blanklen, BLANK, buf[0]);
    }
  } else {
  ...
  edit_grp (fh.filename, lookgrp, fh.title + 8, genbuf);
  substitute_record (BOARDS, &fh, sizeof (fh), pos);
  sprintf (genbuf, "È¡Ïû %s µÄ %s °æ°æÖ÷Ö°Îñ", lookupuser.userid,
	   fh.filename);
  ...
  if (askyn ("Õı³£ÀëÈÎÇë°´ Enter ¼üÈ·ÈÏ£¬³·Ö°³Í·£°´ N ¼ü", YEA, NA) == YEA)
    ...
    right = 0;
  if (right)
    sprintf (bmfilename, "%s °æ%s %s ÀëÈÎÍ¨¸æ",
	     bname, bm ? "°æÖ÷" : "°æ¸±", lookupuser.userid);
  else
    sprintf (bmfilename, "[Í¨¸æ]³·³ı %s °æ%s %s",
	     bname, bm ? "°æÖ÷" : "°æ¸±", lookupuser.userid);
  strcpy (currboard, bname);
  if (right) {
    sprintf (genbuf, "\n\t\t\t¡¾ Í¨¸æ ¡¿\n\n"
	     ...
	     "\tÏ£Íû½ñºóÒ²ÄÜÖ§³Ö±¾°æµÄ¹¤×÷.\n",
	     lookupuser.userid, bname, bm ? "°æÖ÷" : "°æ¸±", bname);
  } else {
    sprintf (genbuf, "\n\t\t\t¡¾³·Ö°Í¨¸æ¡¿\n\n"
	     "\t¾­Õ¾ÎñÎ¯Ô±»áÌÖÂÛ¾ö¶¨£º\n"
	     "\t³·³ı %s °æ%s %s µÄ%sÖ°Îñ¡£\n",
...
m_newbrd ()
{
  ...
  if (getbnum (newboard.filename) > 0
      || mkdir (genbuf, 0755) == -1 || mkdir (vbuf, 0755) == -1) {
    prints ("\n´íÎóµÄÌÖÂÛÇøÃû³Æ!!\n");
    pressreturn ();
    clear ();
    return -1;
  ...
  if (askyn ("ÊÇ·ñÏŞÖÆ´æÈ¡È¨Á¦", NA, NA) == YEA) {
    ...
    move (2, 0);
    prints ("Éè¶¨ %s È¨Á¦. ÌÖÂÛÇø: '%s'\n",
	    (newboard.flag & BOARD_POST_FLAG ? "POST" : "READ"),
	    newboard.filename);
    newboard.level =
      setperms (newboard.level, "È¨ÏŞ", NUMPERMS, showperminfo);
    clear ();
  ...
  if (askyn("ÊÇ·ñ ¿ÉÒÔ ZAPÌÖÂÛÇø£¿",
	       (newboard.flag & BOARD_NOZAP_FLAG) ? NA : YEA, YEA) == NA) {
...
m_editbrd ()
{
  ...
  while (1) {
    ...
    prints ("4)ĞŞ¸Ä%sËùÊôÄ¿Â¼:        %s(%d)\n",
	    type, bcache[fh.group - 1].filename, newfh.group);
    if ( fh.flag & BOARD_DIR_FLAG ) {
	  ...
	  prints ("5)ĞŞ¸Ä%s¶ÁĞ´ÊôĞÔ:        %s\n", type,
	    (newfh.flag & BOARD_POST_FLAG) ? "p(ÏŞÖÆ·¢ÎÄ)" :
	    (newfh.level == 0) ? "Ã»ÓĞÏŞÖÆ" : "r(ÏŞÖÆÔÄ¶Á)");
	}
	  //zap dir and board. Danielfree 06.2.22
	  prints ("6)¿ÉÒÔZAP%s:             %s\n",type,
	      (newfh.flag & BOARD_NOZAP_FLAG) ? "¿É" : "·ñ");
    if (!(newfh.flag & BOARD_DIR_FLAG)) {
      prints ("7)ÒÆ¶¯¾«»ªÇøÎ»ÖÃ\n");
      //prints ("7)¿ÉÒÔZAP°æÃæ:             %s\n",
	  //    (newfh.flag & BOARD_POST_FLAG) ? "¿É" : "·ñ");
      prints ("8)ÄäÃû°æÃæ:                %s\n",
	      (newfh.flag & BOARD_ANONY_FLAG) ? "ÄäÃû" : "²»ÄäÃû");
      prints ("9)¿ÉÒÔ»Ø¸´:                %s\n",
	      (newfh.flag & BOARD_NOREPLY_FLAG) ? "²»¿É»Ø¸´" : "¿ÉÒÔ»Ø¸´");
      prints ("A)ÊÇ·ñ¼ÆËãÎÄÕÂÊı:          %s\n",
	      (newfh.flag & BOARD_JUNK_FLAG) ? "²»¼ÆËã" : "¼ÆËã");
      prints ("B)¾ãÀÖ²¿ÊôĞÔ:              %s\n",
	      (newfh.flag & BOARD_CLUB_FLAG) ? 
	      (newfh.flag & BOARD_READ_FLAG) ?"\033[1;31mc\033[0m(¶ÁÏŞÖÆ¾ãÀÖ²¿)" :
	      	      "\033[1;33mc\033[0m(ÆÕÍ¨¾ãÀÖ²¿)" : "²»ÊÇ¾ãÀÖ²¿");
#ifdef ENABLE_PREFIX
	...
	if (askyn
	    ("ĞŞ¸ÄÌÖÂÛÇø¹ÜÀíÔ±¡£×¢Òâ£º½ö¹©³ö´íĞŞÕıÊ¹ÓÃ£¬°æÖ÷ÈÎÃâÇëÎğ¸Ä¶¯´Ë´¦£¡",
	     NA, NA) == YEA) {
	  getdata (16, 0, "ÌÖÂÛÇø¹ÜÀíÔ±: ", newfh.BM, sizeof (newfh.BM),
		   DOECHO, YEA);
	  if (newfh.BM[0] == '\0') {
	...
	if (askyn ("±¾°æ³ÏÕ÷°æÖ÷Âğ(·ñ£¬ÔòÓÉSYSOPs¹ÜÀí)?", YEA, NA) == NA) {
      ...
      break;
    case '4':
      if (gettheboardname (15, "ÊäÈëËùÊôÌÖÂÛÇøÃû: ", &tmppos, &tmpfh, genbuf, 2))
	newfh.group = tmppos;
	...
	move (2, 0);
	prints ("Éè¶¨ %s '%s' ÌÖÂÛÇøµÄÈ¨ÏŞ\n",
		newfh.flag & BOARD_POST_FLAG ? "ÕÅÌù" : "ÔÄ¶Á",
		newfh.filename);
	newfh.level = setperms (newfh.level, "È¨ÏŞ", NUMPERMS, showperminfo);
	clear ();
      }
	...
	//both dir and board can zap. Danielfree 06.2.22
	  ...
	  if (askyn
	      ("ÊÇ·ñ ¿ÉÒÔ ZAPÌÖÂÛÇø£¿",
	       (fh.flag & BOARD_NOZAP_FLAG) ? NA : YEA, YEA) == NA) {
	...
	case '8':
	  if (askyn
	      ("ÊÇ·ñÄäÃû°æ£¿", (fh.flag & BOARD_ANONY_FLAG) ? YEA : NA, NA)
	      == YEA) {
	  ...
	  break;
	case '9':
	  if (askyn ("ÎÄÕÂÊÇ·ñ ¿ÉÒÔ »Ø¸´£¿", (fh.flag & BOARD_NOREPLY_FLAG) ? NA : YEA, YEA) == NA) {
	...
	case 'A':
	  if (askyn
	      ("ÊÇ·ñ ²»¼ÆËã ÎÄÕÂÊı£¿", (fh.flag & BOARD_JUNK_FLAG) ? YEA : NA,
	       NA) == YEA) {
	...
	case 'B':
	  if (askyn
	      ("ÊÇ·ñ¾ãÀÖ²¿°æÃæ£¿", (fh.flag & BOARD_CLUB_FLAG) ? YEA : NA, NA)
	      == YEA) {
	    newfh.flag |= BOARD_CLUB_FLAG;
	    if (askyn
		("ÊÇ·ñ¶ÁÏŞÖÆ¾ãÀÖ²¿£¿", (fh.flag & BOARD_READ_FLAG) ? YEA : NA,
		 NA) == YEA) {
...
// Åú×¢²áµ¥Ê±ÏÔÊ¾µÄ±êÌâ
  void
                  regtitle () {
    prints
      ("[1;33;44mÅú×¢²áµ¥ NEW VERSION wahahaha                                                   [m\n");
    prints
      (" Àë¿ª[[1;32m¡û[m,[1;32me[m] Ñ¡Ôñ[[1;32m¡ü[m,[1;32m¡ı[m] ÔÄ¶Á[[1;32m¡ú[m,[1;32mRtn[m] Åú×¼[[1;32my[m] É¾³ı[[1;32md[m]\n");

    prints
      ("[1;37;44m  ±àºÅ ÓÃ»§ID       ĞÕ  Ãû       Ïµ±ğ             ×¡Ö·             ×¢²áÊ±¼ä     [m\n");
  }
...
//      ÔÚÅú×¢²áµ¥Ê±ÏÔÊ¾µÄ×¢²áIDÁĞ±í
  char           *regdoent (int num, REGINFO * ent) {
    ...
    sprintf (buf, "  %4d %-12s %-12s %-16s %-16s %s",
	     num, ent->userid, rname, dept, addr, datestring);
    return buf;
  }

//      ·µ»Øuserid Óëent->useridÊÇ·ñÏàµÈ
  int
                  filecheck (REGINFO * ent, char *userid) {
    return !strcmp (ent->userid, userid);
  }

// É¾³ı×¢²áµ¥ÎÄ¼şÀïµÄÒ»¸ö¼ÇÂ¼
  int
                  delete_register (int index, REGINFO * ent, char *direct) {
...
//      Í¨¹ı×¢²áµ¥
  int
                  pass_register (int index, REGINFO * ent, char *direct) {
    int             unum;
    struct userec   uinfo;
    char            buf[80];
    FILE           *fout;
...
//      ´¦Àí×¢²áµ¥
  int
                  do_register (int index, REGINFO * ent, char *direct) {
    int             unum;
    struct userec   uinfo;
    //char ps[80];
    register int    ch;
    static char    *reason[] =
      { "ÇëÈ·ÊµÌîĞ´ÕæÊµĞÕÃû.", "ÇëÏêÌîÑ§Ğ£¿ÆÏµÓëÄê¼¶.",
      "ÇëÌîĞ´ÍêÕûµÄ×¡Ö·×ÊÁÏ.", "ÇëÏêÌîÁªÂçµç»°.",
      "ÇëÈ·ÊµÌîĞ´×¢²áÉêÇë±í.", "ÇëÓÃÖĞÎÄÌîĞ´ÉêÇëµ¥.", "ÆäËû"
    };
    ...
    prints
      ("[1;33;44m ÏêÏ¸×ÊÁÏ                                                                      [m\n");
    prints
      ("[1;37;42m [.]½ÓÊÜ [+]¾Ü¾ø [d]É¾³ı [0-6]²»·ûºÏÔ­Òò                                       [m");
...
//  ½øÈë ×¢²áµ¥²ì¿´À¸,¿´Ê¹ÓÃÕßµÄ×¢²á×ÊÁÏ»ò½ø×¢²áµ¥¹ÜÀí³ÌĞò
  int
    ...
    for (;;) {
      getdata (1, 0, "(0)Àë¿ª  (1)Éó²éĞÂ×¢²á (2)²éÑ¯Ê¹ÓÃÕß×¢²á×ÊÁÏ ? : ",
	       ans, 2, DOECHO, YEA);
...
//rewrite by iamfat 2002.08.19
  extern void     list_text (char *fname,
			     void (*title_show) (),
			     int (*key_deal) (), int (*ifcheck) ()
    );
...
//ÏÔÊ¾´¦·£µ½ÆÚµÄIDÁĞ±í±êÌâ.
  void
    ...
    move (0, 0);
    prints
      ("[1;44;36m ´¦·£µ½ÆÚµÄIDÁĞ±í                                                              [m\n");
    prints
      (" Àë¿ª[[1;32m¡û[m] Ñ¡Ôñ[[1;32m¡ü[m,[1;32m¡ı[m] Ìí¼Ó[[1;32ma[m]  ĞŞ¸Ä[[1;32mc[m] »Ö¸´[[1;32md[m] µ½ÆÚ[[1;32mx[m] ²éÕÒ[[1;32m/[m]\n");
    prints
      ("[1;44m ÓÃ»§´úºÅ     ´¦·£ËµÃ÷(A-Z;'.[])                 È¨ÏŞ ½áÊøÈÕÆÚ   Õ¾Îñ          [m\n");
  }

// ´¦ÀíÏÔÊ¾·â½ûÁĞ±íÊ±µÄ°´¼ü´¦Àí
  int
                  denylist_key_deal (char *fname, int ch, char *line) {
    switch (ch) {
    case 'a':			//Ìí¼Ó
      add_denylevel (0);
      break;
...
//      Ìí¼Óµ½·â½ûÁĞ±í
  int
	...
	sprintf (msg, "%sÒò:\n%s\nÓ¦±»·â½û %s È¨ÏŞÖÕÉí\n\nÈç²»·ş±¾¾ö¶¨,"
		 ...
		 "P.S.: %s\n\nÖ´ĞĞÈË: %s\n",
		 uident, getdetailreason (), clevel (ch), ps,
		 currentuser.userid);
      } else if (day > 0) {
	tmtime = localtime (&tt);
	sprintf (strtosave, "%-12s %-34s %-4s %04d.%02d.%02d %-12s\n",
		 uident, getreason (), clevel (ch), 1900 + tmtime->tm_year,
		 tmtime->tm_mon + 1, tmtime->tm_mday, currentuser.userid);
	sprintf (msg,
		 "%sÒò:\n%s\nÓ¦±»·â½û %s È¨ÏŞ%dÌì\n\nÇëÔÚ´¦·£ÆÚÂúºó(%04d.%02d.%02d),"
		 " Ïò%sĞ´ĞÅÒªÇó½â³ı´¦·£.\nÈç²»·ş±¾¾ö¶¨,"
		 " ¿ÉÒÔÁªÏµ´¦·£¾ö¶¨ÈË»òÔÚ7ÈÕÄÚµ½AppealÉêÇë¸´Òé¡£\n"
		 "P.S.: %s\n\nÖ´ĞĞÈË: %s\n", uident, getdetailreason (),
		 clevel (ch), day, 1900 + tmtime->tm_year,
		 tmtime->tm_mon + 1, tmtime->tm_mday, currentuser.userid,
...
//      ´Ó·â½ûÁĞ±íÖĞÊÍ·Å
  int
                  del_from_denylist (char *fname, char *line) {
    FILE           *fpr, *fpw;
    int             deleted = NA;
...
//  ĞŞ¸Ä·â½ûÈ¨ÏŞ
  int
	...
	case '3':
	  ...
	  sprintf (secu, "[Õ¾ÄÚ¹«¸æ]·â½û%sÓëËûÈËÁÄÌìµÄÈ¨Á¦",
		   lookupuser.userid);
  ...
  void
    ...
    prints ("±àĞŞÏµÍ³µµ°¸\n\n");
    for (num = 0;
	 (HAS_PERM (PERM_ESYSFILE)) ?
	 e_file[num] != NULL && explain_file[num] != NULL :
	 explain_file[num] != "menu.ini"; num++) {
...
//      ÈÏÖ¤ÎÄÕÂ?  ËÆºõÃ»ÓĞÊ¹ÓÃµ½¹ı
  void
                  certify_title_show () {
    move (0, 0);
    prints
      ("[1;44;36m ´ıÅú×¼µÄÎÄÕÂÁĞ±í                                                              [m\n");
    prints (" Àë¿ª[[1;32m¡û[m] Ñ¡Ôñ[[1;32m¡ü[m,[1;32m¡ı[m]"
	    " ÅúÔÄ[[1;32ma[m] É¾³ı[[1;32md[m] ²éÕÒ[[1;32m/[m]\n");
    prints ("[1;44m ÓÃ»§´úºÅ     °æÃæ             ÎÄ¼şÃû    "
	    "                                      [m\n");
  }

//ÈÏÖ¤Ê±°´¼ü´¦Àíº¯Êı,ËÆºõÃ»ÓĞÍË³ö¹¦ÄÜ
  int
	...
	break;
      case 'a':		//Í¨¹ı
	move (1, 0);
	if ((ch == 'a' || ch == 'd') && askyn ("ÄúÈ·¶¨Âğ?", NA, NA) == NA) {
	  certify_title_show ();
	  return 0;
	}
