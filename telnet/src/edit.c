...
void write_header(FILE *fp, int mode) {
	...
	if (in_mail)
		...
		fprintf(fp, "发信人: %s (%s), 信区: %s\n",
				...
				(noname && header.chk_anony) ? "我是匿名天使" : uname, currboard);
	}
	fprintf(fp, "标  题: %s\n", save_title);
	getdatestring(now, NA);
	fprintf(fp, "发信站: %s (%s)", BoardName, datestring);
	if (in_mail)
		fprintf(fp, "\n来  源: %s\n", fromhost);
	else
...
int write_file(char *filename, int write_header_to_file, int addfrom,
	...
	if ((uinfo.mode==POSTING||uinfo.mode==SMAIL||uinfo.mode==EDIT)
		...
		if (!dashf(fname) || currentuser.signature == 0 || noidboard)
			fputs("--\n", fp);
		fprintf(fp, "[m[1;%2dm※ 来源:·%s %s·[FROM: %-.20s][m\n", color,
				BoardName, BBSHOST, (noidboard) ? "匿名天使的家" : fromhost);
	}
	if (aborted != -1)
		fclose(fp);
	currline = NULL;
