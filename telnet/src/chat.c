    ...
    while (c > 0) {
        ...
        if (*bptr == '/') {
            ...
            case 't':
                ...
                sprintf(genbuf, "[1;44;33m ·¿¼ä£º [32m%-12.12s  [5;31;44m%6.6s"
...
int	ent_chat(char   *chatbuf) {
    ...
    while (1) {
        ...
        fixchatid(chatid);
        sprintf(	inbuf, "/! %d %d %s %s %d", uinfo.uid,cuser.userlevel, cuser.userid,
                 chatid, HAS_PERM(PERM_CLOAK) ? uinfo.invisible : 0
               );
        chat_send(cfd, inbuf);
...
int	printuserent(struct user_info *uentp) {
    ...
    if (!uentp->active || !uentp->pid)
        return 0;
    if ((uentp->invisible && !(HAS_PERM(PERM_SEECLOAK) || usernum==uentp->uid)) || isreject(uentp))
        return 0;
...
void query_user(char *arg) {
    ...
    */
    sprintf(msg,
            ...
            "[[1;36m%s[m]"
#endif
            " ÉúÃüÁ¦:[[1;32m%d[m]",(check_query_mail(qry_mail_dir) == 1) ? "ÐÅ" : "  "
#ifdef SHOWEXP
            ,exp
#endif
            ,cexpstr(exp)
...
void	chat_date() {
    ...
    sprintf(genbuf, "[1m %s±ê×¼Ê±¼ä: [32m%s[m", BoardName, datestring);
...
void	set_rec() {
    ...
    if (recflag == 0) {
        ...
        clrtoeol();
        transPERstr(buftopic,tmptopic);
        sprintf(genbuf, "[1;44;33m ·¿¼ä£º [32m%-12.12s  [5;31;44m%6.6s[0;1;44;33m"
...
struct chat_command chat_cmdtbl[] = {
                                        ...
                                        {"mail",chat_showmail},
                                        {"v",chat_showmsg}, /* added by Seaman */
                                        {NULL, NULL}
                                    };
