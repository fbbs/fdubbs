...
mailall()
{
	...
	strcpy(doc[2], "(3) ËùÓÐµÄ°æÖ÷");
	strcpy(doc[3], "(4) ËùÓÐÏÖÈÎÕ¾Îñ");
    ...
    /***********Type 2µÄÈºÐÅ¸ÄÎª¹²ÏíÎÄ¼þµÄÐÎÊ½£¬ Ä¿µÄ¼õÉÙÎÄ¼þµÄ¿½±´£¬·ÀÖ¹ËÀ»ú*****************/
    /***********Ïà¹Ø¸Ä¶¯ÎÄ¼þ£ºlist.c, bbs.c***************************************************/
    if(ans[0] - '0' == 2)
        sprintf(fname, "sharedmail/mailall.%s.%d", currentuser.userid, time(0));
    /**********Modified end**********/
...
{
    /****ÅÐ¶ÏÊÇ·ñÎªsharedmail£¬Èç¹ûÊÇÔò´Ó¹²ÏíÎÄ¼þ¶ÁÈ¡****/
...
int	m_new()
{
   ...
   apply_record(currmaildir, read_new_mail, sizeof(struct fileheader),0,1,0);
   while (delcnt--)
...
void mailtitle()
{
    ...
    showtitle( "ÐÅ¼þÑ¡µ¥    ", BoardName );
     prints( " Àë¿ª[[1;32m¡û[m,[1;32me[m] Ñ¡Ôñ[[1;32m¡ü[m, [1;32m¡ý[m] ÔÄ¶ÁÐÅ¼þ[[1;32m¡ú[m,[1;32mRtn[m] »Ø ÐÅ[[1;32mR[m] ¿³ÐÅ£¯Çå³ý¾ÉÐÅ[[1;32md[m,[1;32mD[m] ÇóÖú[[1;32mh[m][m\n" );
	 //Modified by IAMFAT 2002-05-26
     //prints("[1;44m±àºÅ   ·¢ÐÅÕß       ÈÕ ÆÚ      ±êÌâ  ([33mÄúµÄÐÅÏäÈÝÁ¿Îª[%4dK]£¬µ±Ç°ÒÑÓÃ[%4dK][37m) [m\n",total,used);
       prints("[1;44m ±àºÅ   ·¢ÐÅÕß        ÈÕÆÚ   ±êÌâ    ([33mÄúµÄÐÅÏäÈÝÁ¿Îª[%5dK]£¬µ±Ç°ÒÑÓÃ[%4dK][37m) [m\n",total,used);
    clrtobot() ;
}
...
int getmailboxsize(unsigned int userlevel)
{
	...
	if(userlevel&(PERM_XEMPT)) return MAILBOX_SIZE_BM; 
...
{
	if(userlevel&(PERM_SYSOPS)) return MAX_SYSOPMAIL_HOLD;
	if(userlevel&(PERM_LARGEMAIL)) return MAX_SYSOPMAIL_HOLD;
	if(userlevel&(PERM_XEMPT)) return MAX_BMMAIL_HOLD;
	if(userlevel&(PERM_BOARDS)) return MAX_BMMAIL_HOLD;
	if(userlevel&(PERM_REGISTER)) return MAX_MAIL_HOLD;
...
char   * maildoent(int num, struct fileheader *ent)
{
	...
	struct tm *mytm;
#endif
    /****ÅÐ¶ÏÊÇ·ñÊÇType2µÄ¹²ÏíÎÄ¼þ:ÎÄ¼þÃûsharedmail/mailall.$userid.$time****/
    if(ent->filename[0] == 's')
	...
	strcpy(c2, "[1;36m");
	//Modified by IAMFAT 2002-05-27
	if(toupper(ent->title[0])=='R' && toupper(ent->title[1])=='E' && ent->title[2]==':')
	{
		if(!strcmp(topic, ent->title+4))same = YEA;
		reflag=YEA;
	}
	else
	{
		if(!strcmp(topic, ent->title))same = YEA;
		reflag=NA;
	}
	/*
	...
	ellipsis(title,49);
	sprintf(buf, " %s%4d[m %c %-12.12s %s%6.6s[m  %s%.49s[m", same ?(reflag?c1:c2):""
			,num, status, b2, color, date, same ?(reflag?c1:c2) : "", title);
	/*
...
char   *direct;
{
	...
	case -2:
		...
		break;
	case -3:
		prints("ÄúµÄÐÅÏä³¬ÏÞ£¬ÔÝÊ±ÎÞ·¨Ê¹ÓÃÐÅ¼þ·þÎñ.\n");
		break;
/* Added by Amigo 2002.06.10. To add mail right check. */
	case -4:
		prints("ÄúÃ»ÓÐ·¢ÐÅÈ¨ÏÞ£¬ÔÝÊ±ÎÞ·¨Ê¹ÓÃÐÅ¼þ·þÎñ.\n");
		break;
	case -5:
		prints("¶Ô·½²»ÏëÊÕµ½ÄúµÄÐÅ¼þ.\n");
		break;
/* Add end. */
	default:
		prints("È¡Ïû×ª¼Ä...\n");
...
#endif
{
	...
	while (fgets(genbuf, 255, fin) != NULL) {
		if(filter)  ansi_filter(genbuf, genbuf);
	...
	while (1) {
		...
		if (tmp[0] == 'a' || tmp[0] == 'd' || tmp[0] == 'A' || tmp[0] == 'D') {
			...
			if (tmp[0] == 'a' || tmp[0] == 'A')
			     ...
			     * ÈÕ  ÆÚ: 2007.12.19
			     * Î¬»¤Õß: Anonomous
			     * ´úÂë¶Î: ´ÓÏÂÃæwhile(1)Óï¾ä¿ªÊ¼µ½while½áÊø£¬Ò»¹²34ÐÐ¡£
			     * Ä¿  µÄ: Ôö¼ÓÈºÐÅ·¢ÐÅÈËµÄÊ±ºò²»ÐèÒªÃ¿´Î¶¼°´A¼ü£¬ËùÓÐµÄ²Ù×÷Ò»´Î°´A
			     *         Ö®ºóÍê³É¡£
			     * ±¸  ×¢: Õâ¸ö×ö·¨ÆäÊµ²»ÊÇºÜºÃ£¬²»¹ýÒòÎªÕû¸öFBÏµÍ³Éè¼ÆµÄ¾ÖÏÞÐÔ£¬Ã»ÓÐ
			     *         °ì·¨¸Ä³É±È½ÏºÃµÄÁ÷³Ì£¬Ö»ÄÜÔÚÔ­±¾µÄÁ÷³Ì»ù´¡ÉÏÖØ¸´ÀÍ¶¯¡£FBµÄ
			     *         Éè¼ÆÓÐµãÌ«ËÀ°å£¬Ã¿´ÎÔö¼Ó·¢ÐÅÈËµÄÊ±ºò¶¼Ö»´¦ÀíÒ»¸öid£¬¶øÇÒÕâ
			     *         ¸ö´¦Àí¹ý³ÌÊÇ¼ÐÔÓÔÚÆäËû²Ù×÷ÖÐ¼äµÄ£¬Õû¸öÁ÷³ÌµÄñîºÏ¶ÈÌ«¸ß£¬Ã»
			     *         °ì·¨²ð·Ö£¬Ö»ºÃ²ÉÈ¡ÏÂÃæµÄ·½Ê½£¬Ã¿´ÎÔö¼Ó·¢ÐÅÈËµÄÊ±ºòÖØ»æÕû¸ö
			     *         ÆÁÄ»£¬²¢ÇÒÍê³ÉÒ»´ÎÌí¼Ó²Ù×÷¡£Ï£ÍûÒÔºó»áÓÐ¸üºÃµÄ°ì·¨¡£-_-||
			     */
			    while(1) {
				clear();
				cnt = listfilecontent(maillists, 3);
				...
				prints("ÏÖÔÚÊÇµÚ %c ¸öÃûµ¥ (0~9)Ñ¡ÔñÆäËûÃûµ¥", current_maillist);
				move(0,0);
				prints("(A)Ôö¼Ó (D)É¾³ý (I)ÒýÈëºÃÓÑ (C)Çå³ýÄ¿Ç°Ãûµ¥ (E)·ÅÆú (S)¼Ä³ö? [S]£º ");
				move(1,0);
		...
		case 'A':
		case 'a':
		        /* ÕâÒ»¶ÎcaseÓ¦¸ÃÓÀÔ¶¶¼Ö´ÐÐ²»µ½£¬ÒòÎªÇ°ÃæµÄ²¿·ÖÒÑ¾­Íê³ÉÁËÐ©²Ù×÷£¬
			 * ±£ÏÕÆð¼û£¬ÔÝÊ±±£Áô¡£
			 * by Anonomous */
			if (!(lookupuser.userlevel & PERM_READMAIL)) {
				move(2, 0);
...
int sharedmail_file(char tmpfile[STRLEN], char userid[STRLEN], char title[STRLEN])
{
   struct fileheader newmessage;
   if(!getuser(userid))
       return -1;
...
/* Add end. */
	...
	if (HAS_PERM(PERM_SETADDR)) {
		prints("±¾Õ¾Ä¿Ç°Ö»Ìá¹©Õ¾ÄÚ×ªÐÅ£¬ÇëÊäÈëÒª×ª¼ÄµÄÕÊºÅÃû¡£\n");
...
*/
	...
	if(internet_mail) 
		if (askyn("ÊÇ·ñ¹ýÂËANSI¿ØÖÆ·û",YEA,NA) == NA ) filter = NA;
