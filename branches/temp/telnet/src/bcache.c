...
void	resolve_boards()
{
	...
	//add end
	if (brdshm == NULL) {
		brdshm = attach_shm2("BCACHE_SHMKEY", 3693, sizeof(*brdshm), &iscreate);
        if (iscreate) {
		initlastpost=1;
		int i, maxi = -1;
		int fd;
...
int load_ucache(int reload)
{
    ...
    if (uidshm == NULL) {
        uidshm = attach_shm2("UCACHE_SHMKEY", 3696, sizeof(*uidshm), &iscreate);
    }
...
int searchuser(char *userid)
{
	...
	resolve_ucache();
	if(0){	//²»Ö´ÐÐ´Ë¾ä
		for (i = 0; i < uidshm->number; i++)	//µÍÐ§µÄÏßÐÔËÑË÷,¿ÉÒÔ¿¼ÂÇÌá¸ßÐ§ÂÊ
...
int 
getuserec(char  *userid, struct userec *u )
{       
	...
	for (n = 0; n < USHM_SIZE; n++) {
		...
		if (uentp->active && uentp->pid) {
			if (kill(uentp->pid, 0) == -1) { 
	    		...
	    		continue;
			}else if(uentp->mode!=BBSNET && now - uentp->idle_time > 30*60 ){
...
int t_search_ulist(	struct 	user_info *uentp,
					int     (*fptr) (),
					int     farg,
					int		show,
					int		doTalk
...
void update_ulist( 	struct	user_info *uentp,
					int		uent
				 )
{
	resolve_utmp();
