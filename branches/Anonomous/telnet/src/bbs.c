...
void printutitle() {
	...
	prints("[1;44m 编 号  使用者代号   %-19s #%-4s #%-4s %8s    %-12s  [m\n",
			...
			HAS_PERM (PERM_SYSOPS) ? "真实姓名" : "使用者昵称",
#else
			"使用者昵称",
#endif
#ifdef ALLOWGAME
...
#else
			(toggle2 == 0) ? "上站" : "文章", (toggle2 == 0) ? "时数" : "信件",
#endif
			HAS_PERM(PERM_SEEULEVELS) ? " 等  级 " : "",
			(toggle1 == 0) ? "最近光临日期" : (toggle1 == 1) ? "最近光临地点"
					: "帐号建立日期");
}
...
void readtitle() {
	...
	char tmp[STRLEN], bmlists[5][13]; //iamfat 2002.08.28  tmp[40]-->tmp[255] 避免溢出

	// bmlists[3][16]-->bmlists[5][13]增加到5个版主
	...
	bnum = 0;
	for (i = 0, j = 0; currBM[i] != '\0' && i < BMNAMELISTLEN; i++) { //将版主名单拷到bmlists里
		if (currBM[i] == ' ') {
			bmlists[bnum][j] = '\0';
			bnum++;
			j = 0;
	...
	prints(" 离开[[1;32m←[m,[1;32me[m] 选择[[1;32m↑[m,[1;32m↓[m] 阅读[[1;32m→[m,[1;32mRtn[m] 发文章[[1;32mCtrl-P[m] 砍信[[1;32md[m] 备忘录[[1;32mTAB[m] 求助[[1;32mh[m]\n");
	if (digestmode == 0) {
	...
	/*
	 ...
	 prints ("[1;37;44m  编号   %-12s %6s %-25s 在线[%4d] [%4s式看版] [m\n", "刊 登 者",
	 "日  期", " 标  题", bp->online_num, readmode);
	 else if (digestmode == 5 || digestmode == 6)
	 prints
	 ("[1;37;44m  编号   %-12s %6s %-9s (关键字: [32m%-12s[37m) [[33m%s[37m同作者阅读] [m\n",
	 "刊 登 者", "日  期", " 标  题", someoneID,
	 (digestmode == 5) ? "模糊" : "精确");
	 else if (digestmode == 7)
	 prints ("[1;37;44m  编号   %-12s %6s %-19s 在线[%4d] [%10s式看版] [m\n",
	 "刊 登 者", "日  期", " 标  题", bp->online_num, readmode);
	 else if(digestmode==TRASH_MODE)
	 prints ("[1;37;44m  编号   %-12s %6s %-25s 在线[%4d] [%6s模式] [m\n", "刊 登 者",
	 "日  期", " 标  题", bp->online_num, readmode);
	 else
	 prints ("[1;37;44m  编号   %-12s %6s %-27s 在线[%4d] [%4s模式] [m\n", "刊 登 者",
	 "日  期", " 标  题", bp->online_num, readmode);
	 */
	if (digestmode == 5 || digestmode == 6)
		prints(
		...
		prints(
				"[1;37;44m  编号   %-12s %6s %-19s            [%10s式看版] [m\n",
				"刊 登 者", "日  期", " 标  题", readmode);
	else if (digestmode == TRASH_MODE)
		prints(
				"[1;37;44m  编号   %-12s %6s %-25s            [%6s模式] [m\n",
				"刊 登 者", "日  期", " 标  题", readmode);
	else {
		if (bp->flag & BOARD_NOREPLY_FLAG) {
			prints(
					"[1;37;44m  编号   %-12s %6s %-8s  [33m本版不可回复[37m    在线:%-4d    [%4s模式] [m\n",
					"刊 登 者", "日  期", " 标  题", bs->inboard, readmode);
		} else {
			prints(
					"[1;37;44m  编号   %-12s %6s %-25s 在线:%-4d    [%4s模式] [m\n",
					"刊 登 者", "日  期", " 标  题", bs->inboard, readmode);
		}
	}
	/*
	 if (DEFINE(DEF_THESIS) && digestmode == 0)
	 prints("[1;37;44m 编号   %-12s %6s %-38s[%4s式看版] [m\n", "刊 登 者", "日  期", " 标  题", readmode);
	 else if (digestmode == 5 || digestmode == 6)
	 prints("[1;37;44m 编号   %-12s %6s %-10s (关键字: [32m%-12s[37m) [[33m%s[37m同作者阅读] [m\n","刊 登 者", "日  期", " 标  题", someoneID,(digestmode==5)?"模糊":"精确");
	 else if (digestmode == 7 )
	 prints("[1;37;44m 编号   %-12s %6s %-32s[%10s式看版] [m\n", "刊 登 者", "日  期", " 标  题", readmode);
	 else
	 prints("[1;37;44m 编号   %-12s %6s %-40s[%4s模式] [m\n", "刊 登 者", "日  期", " 标  题", readmode);
	 */
	//Modify End
	clrtobot();
}
...
int read_post(int ent, struct fileheader *fileinfo, char *direct) {
	...
	if (digestmode == ATTACH_MODE) {
		...
		if (haspostperm(currboard)) {
			...
			prints("[1;44;31m[阅读文章]  [33m回信 R │ 结束 Q,← │上一封 ↑│下一封 <Space>,↓│主题阅读 ^s或p [m");
		} else {
			//prints("[1;44;31m[阅读文章]  [33m结束 Q,← │上一封 ↑│下一封 <Space>,<Enter>,↓│主题阅读 ^X 或 p [m");
			prints("[1;44;31m[阅读文章]  [33m结束 Q,← │上一封 ↑│下一封 <Space>,<Enter>,↓│主题阅读 p       [m");
		}
...
int do_select(int ent, struct fileheader *fileinfo, char *direct) {
	...
	if ((bp->flag & BOARD_CLUB_FLAG) && (bp->flag & BOARD_READ_FLAG)
			&& !chk_currBM(bp->BM, 1) && !isclubmember(currentuser.userid,
			bname)) {
		clear();
		move(5, 10);
...
int post_cross(char islocal, int mode) {
	...
	bp = getbcache(currboard);
	if ((bp->flag & BOARD_CLUB_FLAG) && !chk_currBM(bp->BM, 1)
...
void add_crossinfo(char *filepath, int mode) {
	...
	fprintf(fp, "--\n[m[1;%2dm※ 转%s:·%s %s·[FROM: %-.20s][m\n", color,
			(mode == 1) ? "载" : "寄", BoardName, BBSHOST, fromhost);
	fclose(fp);
	return;
}
...
int makeDELETEDflag(int ent, struct fileheader *fileinfo, char *direct) {
	if (!(chk_currBM(currBM, 0)) || fileinfo->accessed[0] & (FILE_MARKED
			| FILE_DIGEST)) {
		return DONOTHING;
	}
#ifdef ENABLE_NOTICE
 ...
 *dst='\0';
 ...
 char ycul_text[1024];
 clear();
 getdata(0, 0, "您想查找的关键字: ", key, 30, DOECHO, YEA);
