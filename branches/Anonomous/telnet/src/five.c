...
void InitScreen()
{
	...
	user_guide();
	move(0,33);
	outs("[35;43m◆五子棋对战◆[30;42m  程式:成大电机88级 Birdman  [m");
}
...
void win( int who )
{
	...
	move(13,35);
	if( who == black )
	outs("[47m[31m│  [30;42m黑胜[m[47m [31m │[m");
	else
	outs("[47m[31m│  [30;42m白胜[m[47m [31m │[m");
	move(14,35);
	outs("[47m[31m└————┘[m");
	refresh ();
	winner=who;
...
void quit(void)
{
	...
	outs("[47m[31m┌———————┐[m");
	move(13,35);
	outs("[47m[31m│  [30;42m对方退出了[m[47m [31m │[m");
	move(14,35);
	outs("[47m[31m└———————┘[m");
	refresh ();
	bell();
		...
		int x4,int y4,int x5,int y5,int x6,int y6)
{
	...
	if(playboard[x1][y1]==black) n_black+=1; /*check 连子*/
...
int bandhand(int style)
{
	if(style==3)
	{
		...
		outs("[47m[31m┌黑败——————┐[m");
		move(13,35);
		outs("[47m[31m│  [37;41m黑禁手双活三[m[47m  [31m│[m");
		move(14,35);
		outs("[47m[31m└————————┘[m");
	}
	else if(style==4)
	{
		...
		outs("[47m[31m┌黑败——————┐[m");
		move(13,35);
		outs("[47m[31m│  [37;41m黑禁手双  四[m[47m  [31m│[m");
		move(14,35);
		outs("[47m[31m└————————┘[m");
	}
	else
	{
		...
		outs("[47m[31m┌黑败——————┐[m");
		move(13,35);
		outs("[47m[31m│  [37;41m黑禁手连六子[m[47m  [31m│[m");
		move(14,35);
		outs("[47m[31m└————————┘[m");
	}
	...
	while (1)
	{
		...
		if (ch == I_OTHERDATA)
		{
			...
			if (data[0] == '\0') {
				...
				RMSG = YEA;
				saveline (PROMPT, 0);
				sprintf (genbuf, "%s 说: 重来一盘好吗? (Y/N)[Y]:", save_page_requestor);
				...
				move (PROMPT, 0);
				clrtoeol ();
				prints ("%s 接受了您的请求 :-)", save_page_requestor);
				refresh ();
				sleep (1);
				saveline (PROMPT, 1);
				InitScreen ();
				...
				move (PROMPT, 0);
				clrtoeol ();
				prints ("%s 拒绝了您的请求 :-(", save_page_requestor);
				refresh ();
				sleep (1);
				saveline (PROMPT, 1);
				if (chating)
			...
			if (ch == Ctrl('X')) {
				...
				RMSG = YEA;
				saveline (PROMPT, 0);
				getdata(PROMPT, 0, "您确定要离开吗? (Y/N)?[N] ", genbuf1, 2, LCECHO, YEA);
				...
				saveline (PROMPT, 0);
				RMSG = YEA;
				getdata(PROMPT,0,"您确定要重新开始吗? (Y/N)?[N] ",genbuf1,2,LCECHO, YEA);
				if (genbuf1[0] == 'Y' || genbuf1[0] == 'y') {
					send (fd, "\1", 1, 0);
					move (PROMPT, 0);
					bell ();
		...
		if(fdone==1 && !chating && ch != I_OTHERDATA)/*换我*/
		{
			...
			switch (ch)
			{
				...
				if((px%2)==1) px=px-1; /*解决netterm不合问题*/
				move(py,px);
				hand+=1;
...
press (void)
{
	...
	move (t_lines-1, 0);
	clrtoeol ();
	prints ("[37;40m[0m                               [33m按任意键继续 ...[37;40m[0m");
	refresh ();
	read (0, &c, sizeof (int));
	move (t_lines-1, 0);
