...
int BM_range(int ent, struct fileheader *fileinfo, char *direct) {
	...
	if (type != 7) {
		sprintf(info, "区段 [%s] 操作范围 [ %d -- %d ]，确定吗", items[type-1],
				num1, num2);
	...
	sprintf(info, "%s版\"L\"区段%s，范围[%d-%d]", currboard, items[type-1],
			num1, num2);
...
char *direct;
{
	...
	getdata(t_lines-2,0,"区段功能:首篇文章编号: ",num1,10,DOECHO,YEA);
	inum1 = atoi(num1);
	if(inum1 <= 0) {
	...
	getdata(t_lines-2,0,"区段功能:末篇文章编号: ",num2,10,DOECHO,YEA);
	inum2 = atoi(num2);
	if(inum2 <= inum1) {
		saveline(t_lines-2, 1);
		return DONOTHING;
...
int search_articles(struct keeploc *locmem, char *query, int gid,
		...
		int aflag,// -1 内容 0 同主题 1 作者  2 相关主题
