...
int print_user_info_title() {
	...
	//   sprintf(title_str,"[1;44m编号 %s使用者代号%s %s%s%s%8.8s %s上站的位置%s      P M %c%s目前动态%s %5s[m\n",
	//   sprintf(title_str,"[1;44m编号 %s使用者代号%s %s%s%s%8.8s %s上站的位置%s      P M %c %s目前动态%s %5s [m\n",
	//Modified by IAMFAT 2002-05-29
	sprintf(
			title_str,
			"[1;44m 编号 %s使用者代号%s %s%s%s%8.8s %s上站的位置%s     P M %c%s目前动态%s  %5s[m\n",
...
int do_userlist() {
	...
	for (i = 0, j = 0; j < BBS_PAGESIZE && i + page < range; i++) {
		...
		if (uentp != NULL) { // by wujian 增加是否为对方好友的显示
			//Added IAMFAT 2002-05-27
			char userid[STRLEN];
			strcpy(userid,
			...
			sprintf(
					...
					(override) ? ((hisfriend(uentp))? "[32m√":"√[32m"):((hisfriend(uentp))?"[33m  ":"  "),
					uentp->userid,userid,
...
int deal_key(char ch, int allnum, int pagenum) //环顾四方处理按键
{
	...
	switch (ch) {
		...
		case 'R':
			...
			sprintf(buf, "确定要把 %s 加入%s名单吗", user_record[allnum]->userid,
					desc);
			move(BBS_PAGESIZE + 2, 0);
			if (askyn(buf, NA, NA) == NA)
				break;
		...
		case 'D':
			if (!strcmp("guest", currentuser.userid))
				return 0;
			sprintf(buf, "确定要把 %s 从好友名单删除吗", user_record[allnum]->userid);
			move(BBS_PAGESIZE + 2, 0);
			if (askyn(buf, NA, NA) == NA)
				break;
...
int deal_key2(char ch, int allnum, int pagenum) //探视网友处理按键
{
	char buf[STRLEN];
	static int msgflag;
...
int star, curr;
{
	if (user_record[curr]->userid != NULL) {
		...
		move(t_lines - 1, 0);
		prints("[0;1;37;44m聊天[[1;32mt[37m] 寄信[[1;32mm[37m] 送讯息[[1;32ms[37m] 加,减朋友[[1;32mo[37m,[1;32md[37m] 选择使用者[[1;32m↑[37m,[1;32m↓[37m] 切换模式 [[1;32mf[37m] 求救[[1;32mh[37m][m");
	}
}
do_query2(star, curr)
int star, curr;
{
	if (user_data[curr - star].userid != NULL) {
		...
		move(t_lines - 1, 0);
		prints("[0;1;37;44m          寄信[[1;32mm[37m] 加,减朋友[[1;32mo[37m,[1;32md[37m] 看说明档[[1;32m→[37m,[1;32mRtn[37m] 选择[[1;32m↑[37m,[1;32m↓[37m] 求救[[1;32mh[37m]          [m");
	}
}
...
int t_friends() {
	...
	if (!dashf(buf)) {
		move(1, 0);
		clrtobot();
		prints("您尚未利用 Info -> Override 设定好友名单，所以...\n");
		range = 0;
	} else {
		num_alcounter();
		range = count_friends;
