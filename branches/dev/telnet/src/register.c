...
int ex_strcmp( char *restrictid, char *userid ) {
    ...
    if( strBuf[ 0 ] == '*' && strBuf[ intLength - 1 ] == '*' ) {
        ...
        return ( strncasecmp( strBuf, strUID, ptr - strBuf ) ||
                 ci_strnbcmp( strBuf, strUID, intLength - 1 - ( ptr - strBuf ) ) );
    }
    return 1;
}
/*
    ...
    */
    ...
    if( (fp = fopen( ".badname", "r" )) != NULL ) {
        while( fgets( buf, STRLEN, fp ) != NULL ) {
            ...
            if( ptr != NULL && *ptr != '#' && ( strcasecmp( ptr, userid ) == 0 || ex_strcmp_v2( ptr, userid ) == 0 ) ) {
                fclose( fp );
                return 1;
            }
        }
...
int compute_user_value(struct userec *urec) {
    ...
    value2 = (time(0) - urec->firstlogin)/60; //×¢²áÊ±¼ä       
/* new user should register in 30 mins */
   ...
   if  ( value2 > 2628000 )  //×¢²áÊ±¼ä´óÓÚ5Äê
        return (666 * 1440 - value) / 1440;
   else if ( value2 >= 	1051200 ) //×¢²áÊ±¼äÔÚ2ÄêºÍ5ÄêÖ®¼ä
...
int getnewuserid() {
    ...
    if (stat("tmp/killuser", &st)==-1 || ( (st.st_mtime+72000 < system_time) && (area->tm_hour < 12) && (area->tm_hour > 5) ) ) {
        ...
        for (i = 0; i < MAXUSERS; i++) {
            ...
            if (utmp.userid[0] != '\0' && val < 0) {
                ...
                if (!bad_user_id(utmp.userid)) {
					...
					getdatestring (utmp.lastlogin, NA);
					fprintf	(fdtmp, "ÉÏ ´Î ÔÚ:[[1;32m%s[m] ´Ó [[1;32m%s[m] µ½±¾Õ¾Ò»ÓÎ¡£\n",
						datestring,(utmp.lasthost[0]=='\0'?"(²»Ïê)":utmp.lasthost));
					...
					, perf
				#endif
					, cperf(perf), (check_query_mail(genbuf) == 1) ? "ĞÅ" : "  ");
					...
					exp, 
				#endif
					cexpstr(exp),compute_user_value(&utmp),(time (0) - utmp.firstlogin) / 86400);
                ...
                */
                substitut_record(PASSFILE, &zerorec,sizeof(struct userec), i+1);
		...
		fclose (fdtmp);
		getdatestring(system_time,NA);
		sprintf(genbuf, "[%8.8s %6.6s] ±¾ÈÕËæ·çÆ®ÊÅµÄID",datestring+6, datestring + 23);							
...
#ifndef FDQUAN
        getdata(22, 0, "ÄúÊÇ·ñ×ĞÏ¸ÔÄ¶Á¹ı±¾Õ¾Announce°æ¾«»ªÇøx-3Ä¿Â¼ËùÁĞÕ¾¹æÖ´ĞĞ°ì·¨²¢±íÊ¾Í¬Òâ[y/N]", verify_code, IDLEN + 1, DOECHO, YEA);
...
//#endif
    ...
    */
    ...
    for (try
        ...
        getdata(0, 0, "ÇëÔÙÊäÈëÒ»´ÎÄúµÄÃÜÂë (Reconfirm Password): ",
                passbuf, PASSLEN, NOECHO, YEA);
    ...
    /* added by roly */
    sprintf(genbuf,"/bin/rm -fr %s/mail/%c/%s", BBSHOME,toupper(newuser.userid[0]),newuser.userid) ;
    system(genbuf);
    sprintf(genbuf,"/bin/rm -fr %s/home/%c/%s", BBSHOME,toupper(newuser.userid[0]),newuser.userid) ;
    system(genbuf);
    /* add end */
...
//   SENDMAIL, trec->userid, BBSHOST, mail);
	...
	fclose(mailp);
	 sprintf(genbuf, "%s -au %s -ap %s -f %s.bbs@%s %s",
	 	  SENDMAIL,mailuser,mailpass,trec->userid, BBSHOST, mail);
	fout = popen(genbuf, "w");
	if (fout != NULL) {
		fprintf(fout, "Reply-To: SYSOP.bbs@%s\n", BBSHOST);
		fprintf(fout, "From: SYSOP.bbs@%s\n", BBSHOST);
...
void check_reg_mail()
{
	...
	if (!dashf(buf)) {
		...
		do {
			...
			if ( invalidaddr(email)||(strstr(email, "@fudan.edu.cn") == NULL)|| invalid_email(email) == 1 ){
	...
	if (buf[0] != 'n' && buf[0] != 'N'){
		...
		if ((fout = fopen(buf, "r")) != NULL) {
			...
			for (i = 0; i < 3; i++) {
				...
				getdata(16,0,"ÇëÊäÈëÈÏÖ¤Âë: ",genbuf,(RNDPASSLEN+1),DOECHO,YEA);
...
int isNumStrPlusX(char *buf)
{
  ...
  for(i = 0; i < strlen(buf); i++)
  {
    if(!(buf[i]>='0' && buf[i]<='9') && !(buf[i] == 'X')) return 0;
  }
  return 1;
}
void check_reg_extra()
{
  ...
  if(!dashf(buf))  
  {
	do {
    	...
    	do
    	{
      	getdata(2,0,"ÊäÈëÒÔÇ°µÄÑ§ºÅ: ", schmate.school_num, SCHOOLNUMLEN+1, DOECHO,YEA);
    	}while(!isNumStr(schmate.school_num));  //Èç¹ûÓĞÊäÈë·ÇÊı×Ö,ÖØĞÂÊäÈë!ÏÂÍ¬
    	do 
    	{		
       	getdata(4,0,"ÊäÈëÓÊÏä(Íâ²¿ÓÊÏäÒà¿É): ", schmate.email, STRLEN, DOECHO, YEA);
   		} while (invalidaddr(schmate.email));
    	do{
      	getdata(6,0, "ÊäÈëÉí·İÖ¤ºÅÂë: ", schmate.identity_card_num, IDCARDLEN+1, DOECHO, YEA);
    	}while(!isNumStrPlusX(schmate.identity_card_num) || strlen(schmate.identity_card_num)!=IDCARDLEN);
    
    	do{
      	getdata(8,0, "ÊäÈë±ÏÒµÖ¤Êé±àºÅ: ", schmate.diploma_num, DIPLOMANUMLEN+1, DOECHO, YEA);
    	}while(!isNumStr(schmate.diploma_num));

    	do{
      	getdata(10,0, "ÊäÈëÊÖ»ú»ò¹Ì¶¨µç»°ºÅÂë: ", schmate.mobile_num, MOBILENUMLEN+1, DOECHO, YEA);
    	}while(!isNumStr(schmate.mobile_num));

    	strcpy(buf, "");
		getdata(11,0,"ÒÔÉÏĞÅÏ¢ÊäÈëÕıÈ·²¢½øĞĞÓÊÏä°ó¶¨ÈÏÖ¤[Y/n]",buf,2,DOECHO,YEA);
	}while (buf[0] =='n' || buf[0] == 'N');
    sprintf(buf, "%s, %s, %s, %s, %s\n", schmate.school_num, schmate.email, schmate.identity_card_num, schmate.diploma_num, schmate.mobile_num);
