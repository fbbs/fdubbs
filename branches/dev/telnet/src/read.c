			...
			)
{
	...
	while ((ch = egetch()) != EOF) {
		if (talkrequest) {
			...
			if(lbc!=0)update_endline();
			lbc = 0;
			mode = i_read_key(rcmdlist, locmem, ch, ssize);

			while (mode == READ_NEXT || mode == READ_PREV || mode == READ_AGAIN) {
				int     reload;
				if(mode==READ_AGAIN)
					reload = 1;
				else
...
int SR_BMfunc(int ent, struct fileheader *fileinfo, char *direct)
{
   ...
   struct keeploc *locmem;
   char SR_BMitems[9][7] = 
      {"É¾³ý", "±£Áô", "ÎÄÕª", "¾«»ªÇø", "Ë®ÎÄ","²»¿ÉRE","ºÏ¼¯","»Ö¸´","ºÏ²¢"};
   char subBMitems[3][9] = 
   ...
   ch[0] = '\0';
   getdata(t_lines - 1, 0, 
      "Ö´ÐÐ: 1) ÏàÍ¬Ö÷Ìâ  2) ÏàÍ¬×÷Õß 3) Ïà¹ØÖ÷Ìâ 0) È¡Ïû [0]: ", 
      ch, 3, DOECHO, YEA);
   dotype = atoi(ch); 
...
int BM_range(int ent, struct fileheader *fileinfo, char *direct)
{
    ...
    * */
   char items[9][8] = 
      {"±£Áô","ÎÄÕª","²»¿ÉRE","É¾³ý","¾«»ªÇø","Ë®ÎÄ","×ªÔØ","É¾Ë®ÎÄ","»Ö¸´"};
...
//   if(strcmp(currboard, "deleted")&&strcmp(currboard,"junk"))max=7;
   ...
   if(((digestmode!=TRASH_MODE && digestmode!=JUNK_MODE)&&((digestmode&&(type==2||type==3||type==9))
      || (digestmode>1 && digestmode!=TRASH_MODE && digestmode!=JUNK_MODE &&type!=5&&type!=6) ))
      || ((digestmode==TRASH_MODE||digestmode==JUNK_MODE)&& type!=5 && type !=6 && type!=9)) {   //modified by phrack :8->9
      saveline(t_lines - 1, 1); 
      return DONOTHING; 
   }
   move(t_lines-1,0);
   ...
   if(type != 7){
      sprintf(info,"Çø¶Î [%s] ²Ù×÷·¶Î§ [ %d -- %d ]£¬È·¶¨Âð",
   ...
   if(type == 5){
      ...
      if(!get_a_boardname(bname,"ÇëÊäÈëÒª×ªÌùµÄÌÖÂÛÇøÃû³Æ: "))return FULLUPDATE;
	 ...
	 return FULLUPDATE;
      ...
      if(askyn("È·¶¨Òª×ªÔØÂð",NA,NA)==NA) return FULLUPDATE;
   }
   ...
   sprintf(info,"%s°æ\"L\"Çø¶Î%s£¬·¶Î§[%d-%d]",
   			currboard, items[type-1], num1, num2);
	   ...
	   switch(type){
           ...
           case 9: 
				  { int result;
					  ...
					  if ((digestmode==TRASH_MODE||digestmode==JUNK_MODE) && result != DONOTHING) {
						  lseek(fdr, (-1)*ssize, SEEK_CUR);
						  num1 --;
						  num2--;
					  }
...
int sread(int readfirst, int auser, struct fileheader *ptitle)
{
   ...
   if (readfirst){
      ...
      move(t_lines - 1, 0);
      clrtoeol();
      prints("[1;44;31m[%8s] [33mÏÂÒ»·â <Space>,<Enter>,¡ý©¦ÉÏÒ»·â ¡ü,U                              [m", auser ? "ÏàÍ¬×÷Õß" : "Ö÷ÌâÔÄ¶Á");
      switch (egetch()) {
         case ' ':
	 case '\n':
	 case KEY_DOWN:
   ...
   while (!isend) {
      if (!isstart) {
         if(search_articles(locmem, title, ptitle->gid, isnext, auser, 0)==-1) break;
      }
      if (uinfo.mode != RMAIL)
	 ...
	 continue;
      ...
      move(t_lines - 1, 0);
      clrtoeol();
      prints("\033[1;44;31m[%8s] \033[33m»ØÐÅ R ©¦ ½áÊø Q,¡û ©¦ÏÂÒ»·â ¡ý,Enter©¦ÉÏÒ»·â ¡ü,U ©¦ ^R »Ø¸ø×÷Õß   \033[m", auser ? "ÏàÍ¬×÷Õß" : "Ö÷ÌâÔÄ¶Á");
      switch (ch = egetch()) {
	 case KEY_LEFT:
         case 'N':
	 case 'Q':
...
int search_articles(struct keeploc *locmem,
		char *query,
		int gid,
		int offset,//-1 µ±Ç°ÏòÉÏ  1 µ±Ç°ÏòÏÂ  3 ×îºóÒ»Æª 5 µÚÒ»Æª
		int aflag,// -1 ÄÚÈÝ 0 Í¬Ö÷Ìâ 1 ×÷Õß  2 Ïà¹ØÖ÷Ìâ
		...
		)
{
	...
	move(t_lines - 1, 0);
	clrtoeol();
	prints("[1;44;33mËÑÑ°ÖÐ£¬ÇëÉÔºò....                                                             [m");
	refresh();
...
int locate_the_post (	struct fileheader *fileinfo,
						char	*query,
						int	offset, //-1 µ±Ç°ÏòÉÏ  1 µ±Ç°ÏòÏÂ  3 ×îºóÒ»Æª 5 µÚÒ»Æª
					...
					)
{
	...
	return search_articles(locmem, query, fileinfo->gid, offset, aflag, newflag);
}
...
int searchallboard (char *id, char *patten, int dt, int all, int del,int flag)
{
	FILE *fp, *fp2, *fp3;
	char f[100], buf2[150];
	char fname[STRLEN];
	...
	while (!feof (fp2))
	{
		...
		if (xx2.flag & BOARD_POST_FLAG || HAS_PERM (xx2.level)
				|| (xx2.flag & BOARD_NOZAP_FLAG))
		{
			int n = 0;
	...
	sprintf(buf2, "[%s]²éÑ¯%sÔÚ%dÌìÄÚ¹Ø¼ü×Ö'%0.10s'",del==1?"°æÃæ":(del==2?"Trash":"Junk"), id, dt/86400, patten);
