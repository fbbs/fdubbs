...
void u_enter()
{
  ...
  if (fn) {
    ...
    substitut_record(PASSFILE, &tmpuserec, sizeof(struct userec), tmpid);
  ...
  while (1) {
    ...
    if (utmpent == -2 && listmode <= 100) {
      listmode++;
      DEBUG(usleep(250));	/* ÐÝÏ¢ËÄ·ÖÖ®Ò»ÃëÔÙ½ÓÔÚÀø */
      continue;
    }
...
{				/*ÕâÐ©ÐÅºÅµÄ´¦ÀíÒª¹Øµô, ·ñÔòÔÚÀëÏßÊ±µÈºò»Ø³µÊ±³öÏÖ  (ylsdd)
  ...
  if (!ERROR_READ_SYSTEM_FILE) {
    ...
    //stay=(stay>7200)?7200/*(14400-stay)*/:stay; //added by iamfat 2002.08.20 ³¬¹ýÁ½¸öÐ¡Ê± ÉÏÕ¾Ê±¼äµÝ¼õ
    set_safe_record();
    now = time(0);
...
void visitlog(void)
{
  ...
  sprintf(genbuf,
	  ...
	  max_log.visit);
    //sprintf(genbuf,
    //       "´Ó [%4dÄê%2dÔÂ%2dÈÕ] Æð, ×î¸ßÈËÊý¼ÇÂ¼: [%d] ÀÛ¼Æ·ÃÎÊÈË´Î: [%u]\n",
    //       max_log.year, max_log.month, max_log.day, max_log.logins,
    //       max_log.visit);
  prints(genbuf);
}
...
void login_query()
{
  ...
  prints("[1;32mÄ¿Ç°ÒÑÓÐÕÊºÅÊý: [[1;36m%d[32m/[36m%d[32m] [32mÄ¿Ç°ÉÏÕ¾ÈËÊý: [[36m%d[32m/[36m%d[1;32m]\n",	// ÆäÖÐWEBÄäÃû: [[36m%d[32m]\n",
	 utmpshm->usersum, MAXUSERS, utmpshm->total_num, 10000);	// get_anon());
  //prints("Ä¿Ç°ÒÑÓÐÕÊºÅÊý: [%d/%d] Ä¿Ç°ÉÏÕ¾ÈËÊý: [%d/%d]\n",   // ÆäÖÐWEBÄäÃû: [^[[36m%d^[[32m]\n",
   //       utmpshm->usersum, MAXUSERS, utmpshm->total_num, 10000);    // get_anon());
...
/*2003.04.22 added by stephen to  add giveup bbs user  login info */
	if (strcasecmp(currentuser.userid, "guest") && !HAS_PERM(PERM_LOGIN)) {
	  ...
	  if (fn) {
	    ...
	    if (tmpuserec.flags[0] & GIVEUPBBS_FLAG && tmpcount == 0)
	      tmpuserec.flags[0] &= ~GIVEUPBBS_FLAG;
	    substitut_record(PASSFILE,
			     &tmpuserec, sizeof(struct userec), tmpid);
	    ...
	    if (sflag[basici][0] == 1) {
	      sprintf(genbuf,
		      "[33mÄúÕýÔÚ½äÍø£¬Àë½äÍø½áÊø»¹ÓÐ%dÌì[m\n",
...
/*02.10.05  add end*/
  ...
  if (!
      (HAS_PERM(PERM_MULTILOG) || (HAS_PERM(PERM_SPECIAL0) && logins < 5)
       || (logins <= MULTI_LOGINS)) && strcmp(currentuser.userid, "guest")) {
...
/*
  ...
  */
  ...
  currentuser.lasthost[15] = '\0';	/* dumb mistake on my part */
  {
     ...
     * ÈÕ  ÆÚ£º2007.12.7
     * Î¬»¤Õß£ºAnonomous
     * ´úÂë¶Ï£º´ÓÏÂÃæµÄif(login_start_time - currentuser.lastlogin > 20 * 60)¿ªÊ¼6ÐÐ¡£
     * ±¸  ×¢£º
     *         Jugde if current user logined within 20 mins since his/her last login.
     *         If so, don't increase his/her numlogins. ;)
     */
    if(login_start_time - currentuser.lastlogin >= 20*60 || !strcmp(currentuser.userid, "guest"))
    {
  ...
  if (allstay < 5) {
    ...
    prints
      ("[1;44;33m[[36m%29s[33m][[36m%4d[33mÈË/[1;36m%3d[33mÓÑ][[36m%1s%1s%1s%1s%1s%1s[33m]ÕÊºÅ%-24s[[36m%3d[33m:[36m%2d[33m][m",
       ...
       (uinfo.invisible == 1) ? "C" : "c", buf, (allstay / 60) % 1000,
       allstay % 60);
    return;
  }
...
void firsttitle(char *title)
{
  ...
  if (chkmail())
    strcpy(middoc,
	   strstr(title,
		  "ÌÖÂÛÇøÁÐ±í") ? "[ÄúÓÐÐÅ¼þ£¬°´ M ¿´ÐÂÐÅ]" : "[ÄúÓÐÐÅ¼þ]");
  else if (mailXX == 1)
...
void c_recover()
{
  ...
  case 's':
    while (1) {
      strcpy(genbuf, "");
      getdata(2, 0, "[1;33mÇëÑ¡ÔñÔÝ´æµµ [0-7] [0]£º[m", genbuf, 2, DOECHO,
	      YEA);
      if (genbuf[0] == '\0')
	a = 0;
      else
	a = atoi(genbuf);
      if (a >= 0 && a <= 7) {
	sprintf(buf, "home/%c/%s/clip_%d", toupper(currentuser.userid[0]),
