    ...
    if (fp = fopen (buf, "r")) {
        ...
        while (1) {
            ...
            for (j = keywords_shm->number - 1; j >= 0; j--) {
                if (strcasestr_zh (buf, keywords_shm->word[j])) {
                    fh->accessed[1] |= FILE_UNCERTIFIED;
...
//added by cometcaptor 2007-7-19 test dir manager
//2007-12-15, ½«resµ÷ÕûÎªstatic, ÐÂÔöchkedboard±äÁ¿
int chk_BM(char *brdstr, int isclub)
{
    char BMstr[BM_LEN];
    static int res = NA;
    static int chkedboard = -1;
    int brdnum = getbnum(brdstr)-1;

    //¼ì²é°æÃæÊÇ·ñÒÑ¸Ä±ä£¬Î´¸Ä±äÔòÖ±½Ó·µ»Øres
    if (brdnum == chkedboard)
        return res;

    //ÈôÒÑ¸Ä±ä°æÃæ£¬½«resÖÃNAºó¿ªÊ¼¼ì²éÊÇ·ñÎª°æÖ÷
    res = NA;
    chkedboard = brdnum;

    while ((res == NA)&&(brdnum>=0))
    {
        if (!(bcache[brdnum].flag & BOARD_POST_FLAG) 
                && !HAS_PERM (bcache[brdnum].level) 
                && !(bcache[brdnum].flag & BOARD_NOZAP_FLAG))
            break;
        strcpy(BMstr, bcache[brdnum].BM);
        res = chk_currBM(BMstr,isclub);
        if ((bcache[brdnum].flag & BOARD_CLUB_FLAG)
                && (bcache[brdnum].flag & BOARD_READ_FLAG )
                && !isclubmember(currentuser.userid, bcache[brdnum].filename))
            break;
        brdnum = bcache[brdnum].group-1;
    }
    return res;
}
...
//·µ»ØÓÃ»§µÄÐÅÏ¢ËùÔÚÄ¿Â¼ÏÂµÄÎÄ¼þ,ÀýÈçÓÃ»§ÃûÎªabc
//              ·µ»ØµÄÊÇ home/A/abc/[filename]
...
//Éè¶¨board°æÃæÏÂfilenameÎÄ¼þµÄÏà¶ÔÂ·¾¶,±£´æÔÚbufÖÐ,²¢·µ»Ø
...
int do_cross (int ent, struct fileheader *fileinfo, char *direct)
{
    ...
    clear ();
    prints
        ("[1;33mÇëÕäÏ§±¾Õ¾×ÊÔ´£¬ÇëÔÚ×ªÔØºóÉ¾³ý²»±ØÒªµÄÎÄÕÂ£¬Ð»Ð»£¡ \n[1;32m²¢ÇÒ£¬Äú²»ÄÜ½«±¾ÎÄ×ªÔØµ½±¾°æ£¬Èç¹ûÄú¾õµÃ²»·½±ãµÄ»°£¬ÇëÓëÕ¾³¤ÁªÏµ¡£[m\n\n");
    prints ("ÄúÑ¡Ôñ×ªÔØµÄÎÄÕÂÊÇ: [[1;33m%s[m]\n", quote_title);
    if (!get_a_boardname (bname,
                "ÇëÊäÈëÒª×ªÌùµÄÌÖÂÛÇøÃû³Æ(È¡Ïû×ªÔØÇë°´»Ø³µ): ")) {
        return FULLUPDATE;
    }
...
void readtitle ()
{
    ...
    prints
        (" Àë¿ª[[1;32m¡û[m,[1;32me[m] Ñ¡Ôñ[[1;32m¡ü[m,[1;32m¡ý[m] ÔÄ¶Á[[1;32m¡ú[m,[1;32mRtn[m] ·¢ÎÄÕÂ[[1;32mCtrl-P[m] ¿³ÐÅ[[1;32md[m] ±¸ÍüÂ¼[[1;32mTAB[m] ÇóÖú[[1;32mh[m]\n");
    if (digestmode == 0) {
    ...
    if (digestmode == 5 || digestmode == 6)
        ...
        prints ("[1;37;44m  ±àºÅ   %-12s %6s %-25s            [%6sÄ£Ê½] [m\n",
                "¿¯ µÇ Õß", "ÈÕ  ÆÚ", " ±ê  Ìâ", readmode);
    else {
...
char   *readdoent (int num, struct fileheader *ent)		// ÎÄÕÂÁÐ±í 
{
    ...
    if (digestmode == ATTACH_MODE) {
        ...
        sprintf (title, "%s %s",
                (digestmode ==
                 2) ? ((ent->accessed[1] & FILE_LASTONE) ? "©¸" : "©À") : "Re:",
                ent->title + 4);
        reflag = 1;
    } else {
    ...
    if (digestmode == TRASH_MODE || digestmode == JUNK_MODE) {
        ...
        if (ent->accessed[1] & FILE_NOTICE) {
            sprintf (buf,
                    " [1;31m [¡Þ][m %c %s %s%17.17s%c%s[%sm%-.49s[m",
                    type,
                    idbuf, color, date, (FFLL & sameflag) ? '.' : ' ',
                    noreply ? "[1;33mx" : " ",
                    (FFLL & sameflag) ? (reflag ? "1;36" : "1;32") : "", title);
...
char   *readdoent (int num, struct fileheader *ent)		// ÎÄÕÂÁÐ±í 
{
    ...
    if (digestmode == TRASH_MODE || digestmode == JUNK_MODE) {
        ...
        sprintf (buf, " %5d %c %-12.12s %s%6.6s\033[m %s", num, type, ent->owner, color, date, title);
    } else {
#ifdef ENABLE_NOTICE
        if (ent->accessed[1] & FILE_NOTICE) {
            sprintf (buf,
                    " [1;31m [¡Þ][m %c %-12.12s %s%6.6s%c%s[%sm%-.49s[m",
                    type,
                    ent->owner, color, date, (FFLL & sameflag) ? '.' : ' ',
                    noreply ? "[1;33mx" : " ",
                    (FFLL & sameflag) ? (reflag ? "1;36" : "1;32") : "", title);
...
#endif
            sprintf (buf,
                    " [%sm%5d[m %c %-12.12s %s%6.6s%c%s[%sm%-.49s[m",
                    (FFLL & sameflag) ? (reflag ? "1;36" : "1;32") : "", num, type,
                    ent->owner, color, date, (FFLL & sameflag) ? '.' : ' ',
                    noreply ? "[1;33mx" : " ",
                    (FFLL & sameflag) ? (reflag ? "1;36" : "1;32") : "", title);
...
int read_post (int ent, struct fileheader *fileinfo, char *direct)
{
    ...
    if (digestmode == ATTACH_MODE) {
        ...
        if (haspostperm (currboard)) {
            ...
            prints
                ("[1;44;31m[ÔÄ¶ÁÎÄÕÂ]  [33m»ØÐÅ R ©¦ ½áÊø Q,¡û ©¦ÉÏÒ»·â ¡ü©¦ÏÂÒ»·â <Space>,¡ý©¦Ö÷ÌâÔÄ¶Á ^s»òp [m");
        } else {
            //prints("[1;44;31m[ÔÄ¶ÁÎÄÕÂ]  [33m½áÊø Q,¡û ©¦ÉÏÒ»·â ¡ü©¦ÏÂÒ»·â <Space>,<Enter>,¡ý©¦Ö÷ÌâÔÄ¶Á ^X »ò p [m");
            prints
                ("[1;44;31m[ÔÄ¶ÁÎÄÕÂ]  [33m½áÊø Q,¡û ©¦ÉÏÒ»·â ¡ü©¦ÏÂÒ»·â <Space>,<Enter>,¡ý©¦Ö÷ÌâÔÄ¶Á p       [m");
        }
    ...
    int do_select (int ent, struct fileheader *fileinfo, char *direct)
    {
        ...
        if ((bp->flag & BOARD_CLUB_FLAG)
                ...
                //&& !chk_currBM (bp->BM, 1)
                && !chk_BM (bp->filename, 1) //cometcaptor 2007-08-11
                && !isclubmember (currentuser.userid, bname))
        {
            clear ();
            move (5, 10);
    ...
    int acction_mode (int ent, struct fileheader *fileinfo, char *direct)
    {
        ...
        if (digestmode != NA) {
            ...
            ch[0] = '\0';
            getdata (t_lines - 1, 0,
                    "ÇÐ»»Ä£Ê½µ½: 1)ÎÄÕª 2)Í¬Ö÷Ìâ 3)±» m ÎÄÕÂ 4)Ô­×÷"
    ...
    int dele_digest (char *dname, char *direc)
    {
        ...
        pos =
            search_record (new_dir, &fh, sizeof (fh), cmpdigestfilename, digest_name);
        if (pos <= 0) {
            return;
        }
    ...
    int digest_post (int ent, struct fileheader *fhdr, char *direct)
    {
        ...
        if (fhdr->accessed[0] & FILE_DIGEST) {
            ...
            if (dashf (genbuf)) {
                fhdr->accessed[0] = fhdr->accessed[0] | FILE_DIGEST;
                if (get_records (direct, &chkfileinfo, sizeof (chkfileinfo), ent, 1) != 1)	// add by quickmouse 01-05-30
                {
                    return DONOTHING;
                }
    ...
    void getcross (char *filepath, int mode)
    {
        ...
        if (mode == 0 || mode == 4) {
            ...
            if (in_mail == YEA)
                ...
                fprintf (of, "[1;37m¡¾ ÒÔÏÂÎÄ×Ö×ªÔØ×Ô [32m%s [37m%sÇø ¡¿[m\n",
                        ((bp->flag & BOARD_POST_FLAG)
                         || (bp->level == 0)) ? quote_board : "Î´Öª",
                        mode ? "¾«»ª" : "ÌÖÂÛ");
                mode = 0;
            }
            if (owner_found) {
    ...
    int show_file_info (int ent, struct fileheader *fileinfo, char *direct)
    {
        ...
        if (in_mail) {
            ...
            snprintf (weblink, 256,
                    "http://%s/cgi-bin/bbs/bbscon?b=%s&f=%s\n",
    ...
    int post_article (char *postboard, char *mailid)
    {
        ...
        if (!haspostperm (postboard) || digestmode == 1
        ...
        if ((bp->flag & BOARD_CLUB_FLAG)
            ...
            clrtobot ();
            prints
                ("\n\n              Äú²»ÊÇ¾ãÀÖ²¿ %s µÄ³ÉÔ±£¬ÇëÏò°æÖ÷ÉêÇë¼ÓÈë¾ãÀÖ²¿",
                 postboard);
            pressreturn ();
            clear ();
            return FULLUPDATE;
    ...
    int change_title (char *fname, char *title)
    {
        ...
        while ((fgets (buf, 256, fp)) != NULL) {
             ...
             * ÈÕ  ÆÚ£º2007.12.20
             * Î¬»¤Õß£ºAnonomous
             * ´úÂë¶Î£º´Ó½ô½Ó×ÅµÄifÓï¾ä¿ªÊ¼µ½ifÓï¾ä½áÊø£¬¹²5ÐÐ¡£
             * ±¸  ×¢£ºÐÞ¸Ä±êÌâµÄÍ¬Ê±£¬ÔÚÎÄÕÂÄ©Î²Ìí¼ÓÒ»¸öÐÞ¸Ä±ê¼Ç£¬±íÃ÷ÎÄÕÂÒÑ¾­±»ÐÞ¸Ä¹ý¡£
             */
            if (!strncmp(buf, "[m[1;36m¡ù ÐÞ¸Ä:¡¤", 17)) {
                snprintf(buf, 256, "[m[1;36m¡ù ÐÞ¸Ä:¡¤%s ì¶ %16.16s ÐÞ¸Ä±¾ÎÄ¡¤[FROM: %-.20s][m\n",
            ...
            if(!found) {
                fprintf(out,  "[m[1;36m¡ù ÐÞ¸Ä:¡¤%s ì¶ %16.16s ÐÞ¸Ä±¾ÎÄ¡¤[FROM: %-.20s][m\n",
                        currentuser.userid, datestring + 6, fromhost);
            }
    ...
    int edit_post (int ent, struct fileheader *fileinfo, char *direct)
    {
        ...
        int edit_title (int ent, struct fileheader *fileinfo, char *direct)
        {
            ...
            if (!chk_BM (currboard, 0)) {//cometcaptor 2007-08-11
                ...
                if (!IsTheFileOwner (fileinfo))
                    return DONOTHING;
                bp = getbcache (currboard);
                if ((bp->flag & BOARD_ANONY_FLAG) && !strcmp (fileinfo->owner, currboard))
                    return DONOTHING;
            }
#ifdef ENABLE_NOTICE
        ...
        int move_notice(int ent, struct fileheader *fh, char *direct)
        {
            ...
            if(fh->accessed[1]&FILE_NOTICE)
            {
                ...
                if (pos > 0)
                {
                    delete_record(path, sizeof(struct fileheader), pos, cmpfilename,
                            fh->filename);
        ...
        int mark_post (int ent, struct fileheader *fileinfo, char *direct)
        {
            ...
            if (get_records (direct, &chkfileinfo, sizeof (chkfileinfo), ent, 1) != 1) {
        ...
        int del_range (int ent, struct fileheader *fileinfo, char *direct)
        {
            ...
            int _UndeleteArticle (int ent, struct fileheader *fileinfo, char  *direct,int response)	/* undelete Ò»ÆªÎÄÕÂ Leeward 98.05.18 */
            {
                char    buf[1024];
                struct stat st;
                int     owned;
                ...
                if (!junkboard () && subflag
                        && owned != 0 && atoi (fileinfo->filename + 2) > lookupuser.firstlogin)
                {
                    lookupuser.numposts++;
                    substitut_record (PASSFILE, &lookupuser, sizeof (struct userec), owned);
                }
            ...
            int _del_post (int ent, struct fileheader *fileinfo, char *direct, int subflag,
                    int hasjudge)
            {
                char    buf[512];
                char    usrid[STRLEN];
                ...
                //if (hasjudge == YEA && !chk_currBM (currBM, 0)) {
                if (hasjudge == YEA && !chk_BM (currboard, 0)) {//cometcaptor 2007-08-11
                    struct boardheader *bp;
                ...
                fail =
                    delete_record (direct, sizeof (struct fileheader), ent, cmpfilename,
                            fileinfo->filename);
                if (!fail) {
                    strcpy (buf, direct);
                    if ((t = strrchr (buf, '/')) != NULL)
...
#define BRC_MAXNUM 60
            ...
            int Save_post (int ent, struct fileheader *fileinfo, char *direct)
            {
                if (!HAS_PERM (PERM_BOARDS) || digestmode == ATTACH_MODE)
                    return DONOTHING;
                //if (!in_mail && !chk_currBM (currBM, 0))
                if (!in_mail && !chk_BM (currboard, 0)) //cometcaptor 2007-08-11
                    return DONOTHING;
                ...
                if (!(bp->flag & BOARD_CLUB_FLAG)
                    ...
                    || !(chk_BM (bp->filename, 1) //cometcaptor 2007-08-11
                        ...
                        struct one_key read_comms[] = {
                            ...
                            '!', Q_Goodbye,
                            'S', s_msg,
                            'f', new_flag_clear,		/* ÇåÈ«²¿Î´¶Á±ê¼Ç  modified by Seaman  ( original: t_friends) */
                        ...
                        int Read ()
                        {
                            ...
                            bm_log (currentuser.userid, currboard, BMLOG_STAYTIME, time (0) - usetime);
                        ...
                        void notepad ()
                        {
                            ...
                            if ((in = fopen (tmpname, "w")) != NULL) {
                                ...
                                while (1) {
                                    ...
                                    getdata (5, 0,
                                            "ÊÇ·ñ°ÑÄúµÄ´ó×÷·ÅÈëÁôÑÔ°å (Y)ÊÇµÄ (N)²»Òª (E)ÔÙ±à¼­ [Y]: ",
                                            note1, 3, DOECHO, YEA);
                                ...
                                if (note1[0] != 'N' && note1[0] != 'n') {
                                    ...
                                    fprintf (in,
                                            "[1;34m¨Š[44m¨‰¨‰¨‰¨‰¨‰¨‰¨‰¨‰¨‰¨‰¨‰¨‰¨‰¨‰¨‰¨‰¨‰[36mËá[32mÌð[33m¿à[31mÀ±[37m°å[34m¨‰¨‰¨‰¨‰¨‰¨‰¨‰¨‰¨‰¨‰¨‰¨‰¨‰¨‰¨‰[44m¨Ž[m\n");
                        ...
                        int Q_Goodbye ()
                        {
                            ...
                            if (dashf (fname)) {
                                ...
                                if (logouts >= 1) {
                                    user_display (fname,
                                            (logouts ==
                                             1) ? 1 : (currentuser.numlogins % (logouts)) + 1, YEA);
                                }
                            } else {
                        ...
                        void canceltotrash (char *path,char *userid, struct fileheader *fh, int subflag, int totrash)
                        {
                            char    tpath[80];
                            char   *ptr;
                        ...
                        int _count_range (char *filename,
                                int from,
                                int to,
                                int sortmode, int sortopt, int numlevel, char *resultfile)
                        {
                            ...
                            while (1) {
                                ...
                                while (count && count->next != tail) {
                                    if ((sortopt == 0 && (sortmode ? (count->all_num > count->next->all_num)
                                                    : (count->all_num < count->next->all_num)))
                                            || (sortopt == 1 && (sortmode ? (count->m_num > count->next->m_num)
                                                    : (count->m_num < count->next->m_num)))
                                            || (sortopt == 2 && (sortmode ? (count->g_num > count->next->g_num)
                                                    : (count->g_num < count->next->g_num)))
                                            || (sortopt == 3 && (sortmode ? (count->w_num > count->next->w_num)
                                                    : (count->w_num < count->next->w_num)))
                                            || (sortopt == 4
                                                && (sortmode ? (count->other_num > count->next->other_num)
                                                    : (count->other_num < count->next->other_num)))) {
                                        if (prev)
                            ...
                            fprintf (fresult,
                                    "[1;44;37m Ê¹ÓÃÕß´úºÅ  ©¦×Ü  Êý©¦ ±»MµÄ©¦ ±»GµÄ©¦ ±»wµÄ©¦ÎÞ±ê¼Ç [m\n");
                            //  fprintf(fresult, "©Ä©¤©¤©¤©¤©¤©¤©à©¤©¤©¤©à©¤©¤©¤©à©¤©¤©¤©à©¤©¤©¤©à©¤©¤©¤©Ì\n");
                            count = head;
                            while (count) {
                                if ((sortopt == 0 && count->all_num >= numlevel)
                                        || (sortopt == 1 && count->m_num >= numlevel)
                                        || (sortopt == 2 && count->g_num >= numlevel)
                                        || (sortopt == 3 && count->w_num >= numlevel)
                                        || (sortopt == 4 && count->other_num >= numlevel))
                                    //      fprintf (fresult, "©§%-12.12s©¦%6d©¦%6d©¦%6d©¦%6d©¦%6d©§\n", count->id,
                                    fprintf (fresult, " %-12.12s  %6d  %6d  %6d  %6d  %6d \n", count->id,
                                            count->all_num, count->m_num, count->g_num, count->w_num,
                                            count->other_num);
                                count = count->next;
                            }
                            fprintf (fresult,
                        ...
                        int count_range (int ent, struct fileheader *fileinfo, char *direct)
                        {
                            ...
                            if (digestmode > 1 && digestmode != TRASH_MODE && digestmode != JUNK_MODE)
                                return DONOTHING;
                            getdata (t_lines - 1, 0, "Ê×ÆªÎÄÕÂ±àºÅ: ", num, 6, DOECHO, YEA);
                            from = atoi (num);
                            if (from <= 0) {
                            ...
                            getdata (t_lines - 1, 0, "ÅÅÐò·½Ê½ (0)½µÐò (1)ÉýÐò ? : ", num, 2, DOECHO,
                                    YEA);
                            sortmode = atoi (num);
                            ...
                            getdata (t_lines - 1, 0,
                                    "ÅÅÐòÑ¡Ïî (0)×ÜÊý (1)±»m (2)±»g (3)±»w (4)Î´±ê¼Ç? : ", num, 2,
                                    DOECHO, YEA);
                            sortopt = atoi (num);
                            if (sortopt < 0 || sortopt > 4)
                                sortopt = 0;
