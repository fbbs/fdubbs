...
//               ·ñÔò,·µ»Ø´óÓÚÁãµÄÖµ
int
canbbsnet (char *bbsnetip, char *fromhost)
{
  FILE           *fp;
...
//      ¸ü¸ÄÓÃ»§µÄÈ¨ÏŞÉè¶¨
unsigned int
setperms (unsigned int pbits, char *prompt, int numbers, int (*showfunc) ()
  )
{
...
x_edits ()
{
  ...
  extern int      WishNum;
  static char    *e_file[] = {
    "plans", "signatures", "notes", "logout", "GoodWish", NULL
  };
  static char    *explain_file[] = {
    "¸öÈËËµÃ÷µµ", "Ç©Ãûµµ", "×Ô¼ºµÄ±¸ÍüÂ¼", "ÀëÕ¾µÄ»­Ãæ", "µ×²¿Á÷¶¯ĞÅÏ¢", NULL
  };
  modify_user_mode (GMENU);
  clear ();
  move (1, 0);
		 ...
		 char *bname, int mode)
{
  ...
  *pos = search_record (BOARDS,
			fh, sizeof (struct boardheader), cmpbnames, bname);
  if (!(*pos)) {
    move (x + 3, 0);
...
//#endif
  ...
  prints ("\n[1;36mÆÁÄ»ÒÑÔÚ[33m %s[36m Ê±±»%sÔİÊ±Ëø×¡ÁË...[m",
...
fill_date ()
{
  ...
  sprintf (genbuf, "¼ÍÄîÈÕ¸üĞÂ, ÏÂÒ»´Î¸üĞÂÊ±¼ä %s", datestring);
  report (genbuf);
